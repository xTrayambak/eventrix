...
def tasks_list():
    ...
    if not auth.startswith("Bearer "):
        return jsonify({"error": "BADTOKEN"}), 401
    ...
    # Return this user's tasks
    tasks = []
    for task in Task.select():
        if task.uid != uid:
            continue
        tasks.append({"name": task.name, "due": task.due})
...
def tasks_create():
    ...
    due = int(task.get("due", 0))

    if not name or not due:
        return jsonify({"error": "BADREQ"}), 400

    Task.insert(Task(name=name, due=due, uid=uid))
    db.commit()
    close_db(db)
    return "", 200
...
def tasks_remove():
    ...
    # Remove only the user's own task with this name (first match)
    for task in Task.select():
        if task.uid == uid and task.name == target:
            Task.delete(Where(Task.name, task.name) & Where(Task.uid, uid))
            db.commit()
            close_db(db)
            return "", 200
