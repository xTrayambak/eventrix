...
function populateDrawer() {
  ...
  avatarBtn.textContent = ((authInfo.username && authInfo.username[0]) || "U").toUpperCase();
}
...
function updateHeaderDates(baseDate) {
  ...
  // Mirror ISO dates onto the .day-col elements so clicks know which date they represent
  const cols = weekGrid.querySelectorAll(".day-col");
  cols.forEach((col, i2) => {
    const d2 = new Date(start);
    d2.setDate(start.getDate() + i2);
    col.dataset.iso = d2.toISOString().slice(0, 10);
  });
}
</div><div class="date">${d.getDate()}</div>`;
    el.dataset.iso = d.toISOString().slice(0, 10);
  });
}
...
function renderEvents() {
  ...
  // Compute visible week range [start, end)
  const start = startOfWeek(current);
  const startMid = new Date(start.getFullYear(), start.getMonth(), start.getDate());
  const endMid = new Date(startMid.getFullYear(), startMid.getMonth(), startMid.getDate());
  endMid.setDate(endMid.getDate() + 7);

  // Render only events inside the visible week, positioned by exact date
  events.forEach((ev) => {
    const evDate = new Date(ev.dateEpoch);
    const evMid = new Date(evDate.getFullYear(), evDate.getMonth(), evDate.getDate());
    if (evMid < startMid || evMid >= endMid) {
      return; // skip events outside current week
    }
    const dayIndex = Math.floor((evMid - startMid) / 86400000);
    ...
    if (typeof openDetailsPopup === "function") {
