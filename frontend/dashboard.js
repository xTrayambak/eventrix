...
var authInfo = new AuthInfo("Administrator", "Test", undefined, 1);
...
async function updateAuthInfo() {
	const resp = await fetch(`${BackendUrl}/auth/login`, { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({ username: authInfo.username, password: authInfo.password }) });
	if (!resp.ok)
	{
	...
	console.log("Logged into account successfully! ", body.details, body.details.role);
	authInfo = new AuthInfo(authInfo.username, authInfo.password, body.token.code, body.token.expires_at, body.details.role);
...
function setLogos(){
  ...
  sideLogo.src = src; topLogo.src = src;
}
themeToggle.addEventListener("click", () => {
  bodyEl.classList.toggle("light");
  themeToggle.textContent = bodyEl.classList.contains("light") ? "🌙 Dark" : "☀ Light";
  setLogos();
});
setLogos();
...
collapsedPlus.addEventListener("click", (e) => {
  e.stopPropagation();
  collapsedCreate.style.display = (collapsedCreate.style.display === "block") ? "none" : "block";
});
...
function handleCreate(type){ alert(`Create ${type} (stub)`); }
...
async function getEvents()
{
	...
	const resp = await fetch(`${BackendUrl}/events/list`, { method: "GET", headers: {"Content-Type": "application/json", "Authorization": `Bearer ${authInfo.token}`}});
	if (!resp.ok)
		return
	...
	console.log("Fetched events list successfully")
	console.log(events);
...
function switchToWeek(){
  currentView = "week";
  viewWeekBtn.classList.add("active"); viewMonthBtn.classList.remove("active");
...
function switchToMonth(){
  currentView = "month";
  viewMonthBtn.classList.add("active"); viewWeekBtn.classList.remove("active");
...
monthPopup.addEventListener("click",(e)=>{
  if(e.target === monthPopup){ monthPopup.classList.add("hidden"); }
});
...
/* ------------ Tasks + Upcoming ------------ */
let tasks = [
  { text:"Finalize venue booking", due:"in 3 days", done:false },
  { text:"Email sponsors", due:"tomorrow", done:false },
  { text:"Print badges", due:"next week", done:true },
];
function paintTasks(){
...
function setTab(which){
  if(which==="calendar"){
    tabCalendar.classList.add("active"); tabTasks.classList.remove("active");
    calendarView.classList.remove("hidden"); tasksView.classList.add("hidden");
  }else{
    tabTasks.classList.add("active"); tabCalendar.classList.remove("active");
    tasksView.classList.remove("hidden"); calendarView.classList.add("hidden");
  }
}
tabCalendar.addEventListener("click",()=>setTab("calendar"));
tabTasks.addEventListener("click",()=>setTab("tasks"));

/* ------------ Final boot: ensure week is default and arrows act by view ------------ */
// switchToWeek();
